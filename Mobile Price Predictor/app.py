import os
import sys
import __main__
import joblib
import numpy as np
import pandas as pd
import streamlit as st
from sklearn.base import BaseEstimator, TransformerMixin
import sklearn.compose._column_transformer as ct
if not hasattr(ct, '_RemainderColsList'):
    class _RemainderColsList(list):
        pass
    ct._RemainderColsList = _RemainderColsList

# ==========================================
# 1. Custom Feature Engineering Transformers
# ==========================================
class BrandValueAdder(BaseEstimator, TransformerMixin):
    def __init__(self):
        self.brand_scores = {
            'Apple': 9.8, 'Samsung': 9.0, 'Google': 8.5, 
            'OnePlus': 8.0, 'Xiaomi': 7.2, 'Oppo': 6.8, 
            'Vivo': 6.8, 'Realme': 6.5, 'Motorola': 6.0
        }
    def fit(self, X, y=None):
        return self
    def transform(self, X):
        X_copy = X.copy()
        X_copy['Brand_Value'] = X_copy['Brand'].map(self.brand_scores).fillna(5.0)
        return X_copy

class ServiceScoreAdder(BaseEstimator, TransformerMixin):
    def __init__(self):
        self.service_scores = {
            'Apple': 9.5, 'Samsung': 9.0, 'OnePlus': 8.0, 
            'Xiaomi': 8.2, 'Oppo': 7.5, 'Vivo': 7.5, 
            'Google': 7.0, 'Realme': 7.0, 'Motorola': 6.5
        }
    def fit(self, X, y=None):
        return self
    def transform(self, X):
        X_copy = X.copy()
        X_copy['Service_Score'] = X_copy['Brand'].map(self.service_scores).fillna(4.0)
        return X_copy

class SpecTierClassifier(BaseEstimator, TransformerMixin):
    def fit(self, X, y=None):
        return self
    def transform(self, X):
        X_copy = X.copy()
        def classify_tier(row):
            ram = row.get('RAM (GB)', 0)
            storage = row.get('Storage (GB)', 0)
            if ram >= 8 and storage >= 256:
                return 'Flagship'
            elif ram >= 6 or storage >= 128:
                return 'Mid-Range'
            else:
                return 'Budget'
        X_copy['Spec_Tier'] = X_copy.apply(classify_tier, axis=1)
        return X_copy

# Bind transformers to __main__ so joblib deserialization succeeds
setattr(__main__, "BrandValueAdder", BrandValueAdder)
setattr(__main__, "ServiceScoreAdder", ServiceScoreAdder)
setattr(__main__, "SpecTierClassifier", SpecTierClassifier)

# ==========================================
# 2. UI Layout & User Input
# ==========================================
st.set_page_config(page_title="Mobile Price Predictor", page_icon="📱", layout="centered")

st.title("📱 Real-World Mobile Price Predictor")
st.write("Predict smartphone market prices using brand tiering and machine learning regression.")
st.divider()

col1, col2 = st.columns(2)

with col1:
    brand = st.selectbox(
        "Brand",
        ["Apple", "Samsung", "Google", "OnePlus", "Sony", "Xiaomi", "Asus", "Oppo", "Vivo", "Realme", "Motorola", "Nokia", "Huawei", "Blackberry", "LG"]
    )
    ram = st.selectbox("RAM (GB)", [2, 3, 4, 6, 8, 12, 16], index=3)
    storage = st.selectbox("Storage (GB)", [32, 64, 128, 256, 512, 1024], index=2)

with col2:
    screen_size = st.slider("Screen Size (inches)", min_value=4.0, max_value=7.5, value=6.5, step=0.1)
    battery = st.slider("Battery Capacity (mAh)", min_value=2000, max_value=7000, value=4500, step=100)
    cameras = st.selectbox("Number of Rear Cameras", [1, 2, 3, 4, 5], index=2)

# ==========================================
# 3. Model Inference Execution
# ==========================================
if st.button("Predict Price", type="primary", use_container_width=True):
    raw_df = pd.DataFrame([{
        "Brand": brand,
        "RAM (GB)": ram,
        "Storage (GB)": storage,
        "Screen Size (inches)": screen_size,
        "Battery Capacity (mAh)": battery,
        "Number of Rear Cameras": cameras
    }])

    # Robust path resolution
    script_dir = os.path.dirname(os.path.abspath(__file__))
    candidates = [
        os.path.join(script_dir, "mobile_price_pipeline.pkl"),
        "mobile_price_pipeline.pkl",
        "Mobile Price Predictor/mobile_price_pipeline.pkl"
    ]
    model_file = next((p for p in candidates if os.path.exists(p)), None)

    if model_file:
        try:
            loaded_model = joblib.load(model_file)
            prediction = loaded_model.predict(raw_df)
            predicted_price = float(prediction[0])

            st.subheader("Price Estimate")
            st.metric(label="Estimated Market Price", value=f"${predicted_price:,.2f}")

            # Confidence range (test set MAE ~$59.39)
            lower_bound = max(50.0, predicted_price - (1.96 * 59.39))
            upper_bound = predicted_price + (1.96 * 59.39)

            if predicted_price >= 700:
                tier_badge = "Premium / Flagship 🔴"
            elif predicted_price >= 300:
                tier_badge = "Mid-Range Value 🟡"
            else:
                tier_badge = "Budget / Entry-Level 🟢"

            st.info(f"**Category:** {tier_badge}\n\n**Expected Range (95% Confidence):** ${lower_bound:,.2f} – ${upper_bound:,.2f}")

        except Exception as e:
            st.error(f"Error executing prediction: {e}")
    else:
        st.error("Model file 'mobile_price_pipeline.pkl' not found.")
